...
const profile = mkdtempSync(join(tmpdir(), "tragedy-looper-manual-life-"));
...
async function waitForServer() {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const response = await fetch(`${origin}/test/manual-life-flow.html`);
...
try {
  ...
  const browser = spawnSync(
    ...
    [
      ...
      "--dump-dom",
      `${origin}/test/manual-life-flow.html`,
    ],
    { encoding: "utf8", timeout: 30_000, maxBuffer: 10 * 1024 * 1024 },
  );

  const output = `${browser.stdout ?? ""}\n${browser.stderr ?? ""}`;
  const flowPassed = browser.stdout?.includes('data-manual-life-result="pass"') &&
  ...
  if (
    ...
    throw new Error(
      `수동 생사 교정 실제 UI 검사가 실패했습니다.\n${output}`,
      browser.error ? { cause: browser.error } : undefined,
    );
  }
  process.stdout.write(
    timedOutAfterResult
      ? "수동 생사 교정 실제 UI 검사 통과 (결과 출력 후 Chrome 강제 종료)\n"
      : "수동 생사 교정 실제 UI 검사 통과\n",
  );
} finally {
  vite.kill("SIGTERM");
