...
- 1수 앞 전경 탐색: P2 하나에서 P3 368,640→10,567 P4류, 실제 전이와
  ...
  폰 즉시 비교 범위가 아니다.
- 기존 `distanceToLoss` 분산 89%/94%/94%는 문자 그대로는 재현되지만 전략적
  판정으로는 폐기했다. `role:keyPerson:*`가 핵심 인물 음모 거리가 아니라 현재 사망
  여부였기 때문이다. First Steps 2의 네 실제 승리 경로를 개별 축으로 재계측하면
  1·2·3일차 모두 불변 후보는 0/100이다. “대부분 대응이 무의미”와 이를 전제한
  날짜별 탐색 설계는 무효이며, 근사 조기 종료를 쓰지 않는 판정만 유지한다.
- 번들 47개 난이도의 `distanceToLoss` 반환 계약 12종은 계약 불일치 0건이었다.
