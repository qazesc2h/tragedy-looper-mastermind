...
### 10-12. 2026-08-03 정보 공개형 우호 능력 A 작업 로그 누락 정정

- A의 네 능력은 `87926c0 Add incident and subplot goodwill ability resolution`
  (2026-08-03 20:09:41 KST)에서 이미 구현되었다.
  - `godlyBeing [우호3]`: 발생 여부와 무관하게 시나리오의 사건 회차를 골라
    범인을 공개한다.
  - `policeOfficer [우호4]`: 이번 루프에 실제 발생한 정확한 사건 회차만 골라
    범인을 공개한다. 예정됐지만 발생하지 않은 회차는 거부한다.
  - `informer [우호5]`: 리더가 시나리오 밖 룰 X를 선언할 수 있고, 각본가는
    실제 사용된 다른 룰 X를 공개한다.
  - `ai [우호3]`: 사건 발생 판정과 발생 이력 기록 없이 AI를 범인으로 사건
    효과만 해결하며, 효과에 필요한 리더 선택도 전달한다.
- 같은 커밋이 `GoodwillDeclaration`, UI의 `data-goodwill-choice` 전달,
  이번 루프 공개 정보 목록, 관련 엔진·UI 모델 테스트를 함께 추가했다.
- 이 커밋이 기존 10-7과 10-8에 반영되지 않아 A가 미착수인 것처럼 남아 있었다.
  해당 두 절의 A 관련 기록보다 이 절과 실제 코드를 우선한다.
- 2026-08-03 재검증에서 기존 엔진 테스트는 신의 미발생 사건 허용, 형사의
  미발생 사건 거부, 정보원의 시나리오 밖 선언을 확인했다. 발생 조건 미충족으로
  지나간 사건도 임시 감사 테스트에서 형사가 거부함을 확인했다.
- `test/ui-goodwill-abilities.test.ts`는 네 능력의 선택지 구성과 사건 선택값
  왕복을 확인하지만, `src/ui/main.ts`의 버튼 클릭부터 `resolveGoodwillAbility()`
  호출까지 실행하는 DOM 통합 테스트는 아직 없다.

### 10-13. 2026-08-03 지속 효과형 우호 능력 B 작업 로그 누락 정정

- B의 두 능력은
  `f777322 Implement loop-long incident suppression and protagonist death blocking`
  (2026-08-03 20:27:23 KST)에서 이미 구현되었다.
  - `henchman [우호3]`: `LoopState.incidentCulpritSuppressedFor`에 하수인을
    기록하고, 하수인이 범인인 사건은 발생 조건 단계에서 차단해 발생 이력도
    남기지 않는다.
  - `soldier [우호5]`: `LoopState.protagonistDeathPreventedBy`에 군인을 기록하고,
    `attemptProtagonistDeath()`가 주인공 사망을 차단한 캐릭터를 `blockedBy`로
    반환한다.
- 살인 청부업자·연인A·병원 사건은 주인공 사망으로 분류되어 군인이 막지만,
  시간 여행자·핵심 인물·친구 및 룰의 패배는 주인공 사망이 아니므로 막지 않는다.
- 하수인이 범인인 나비의 날갯짓은 발생하지 않아 미래 변경 계획이 성립하지 않는다.
  검은 고양이는 사건이 발생하되 효과만 없어 발생 이력이 남고, 미래 변경 계획이
  성립한다.
- 차단 사실은 `publicInformationThisLoop`에 기록하지 않는다.
- 이 커밋이 기존 10-7과 10-8에 반영되지 않아 B가 미착수인 것처럼 남아 있었다.
  해당 두 절의 B 관련 기록보다 이 절과 실제 코드를 우선한다.
