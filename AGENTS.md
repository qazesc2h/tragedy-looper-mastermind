...
- **작업 지시서**: `PROMPTS.md` — 사용자가 작업 단위로 하나씩 준다.
- **검증 절차**: `.claude/skills/verify-hook/SKILL.md` — 훅 구현이 원문과
  일치하는지 대조하는 체크리스트. 훅을 구현할 때마다 이 절차를 따르라.
...
### 2-1. `data/ko-rules-text.json` 은 근거가 아니다

참극<기본>편의 한국어 공식 문장이다. **UI 표시와 사람의 교차 확인용**이며
구현 근거가 아니다. 영문 `source` 와 충돌하면 무조건 `source` 가 이긴다.

이 파일을 읽고 구현을 바꾸지 마라. 차이를 발견하면 `QUESTIONS.md` 에 올려라.
