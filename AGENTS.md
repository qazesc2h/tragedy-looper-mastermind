...
이 도구는 각본가 **혼자** 쓰는 로컬 도구다. 규칙과 상태를 정확히 추적하는 것을
바탕으로, 최종적으로 각본가의 전략 판단을 돕는다.

### 전략 보조에 대하여

이 도구의 최종 목적은 각본가의 전략 판단을 돕는 것이다. 다만 다음 원칙을 지킨다.

- 근거 없는 추천을 하지 마라. 모든 조언에는 계산된 근거가 있어야 한다. 패배
  조건까지의 거리, 정보 누출량, 가설 공간 변화 등이다.
- 규칙에 어긋나는 수를 제안하지 마라. `legal.ts`로 검증해라.
- 각본가의 판단을 대체하지 마라. 선택지와 그 결과를 보여주고 고르는 것은 사람이다.
- "최적"이라고 단언하지 마라. 이 게임은 불완전 정보 게임이며 주인공의 행동을
  예측할 수 없다.
