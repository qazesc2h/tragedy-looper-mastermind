...
### Q7. 사용자 시나리오 「못된 고양이」 — 메이드 시작 장소는 어디인가?
- 제공된 시나리오 데이터에는 메이드의 시작 장소가 없다.
- 캐릭터 데이터의 합법 후보는 도시와 학교이며, 기존 엔진은 복수 후보 중 하나를
  임의로 고르지 않고 `scenario.scriptSpecified["startLocation:servant"]`를 요구한다.
- 현재 처리: 시나리오 선택 화면에서 도시/학교 중 하나를 필수 입력으로 받고 그 게임의
  구조화된 시나리오 값으로 확정한다. 제작자가 의도한 고정 장소가 확인되면 커뮤니티
  시나리오 데이터에 기록하고 이 임시 입력을 제거한다.
